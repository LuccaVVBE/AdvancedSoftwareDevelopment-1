...
public class TestStackGeneriek {
	...
	public <T extends Serializable> void itemsToevoegenVerwijderen (MyStack<T> myStack, T[] items){
		...
		//List<T> itemsReversed = Arrays.asList(items)
		Collections.reverse(Arrays.asList(items)); 
		Stream.of(items).limit(items.length-1).forEach(x->{
			Assertions.assertEquals(x, myStack.pop());
			Assertions.assertFalse(myStack.isEmpty());
		});
		Assertions.assertEquals(items[items.length-1], myStack.pop());
