...
public class ReductiebonBeheerder {
	...
	public double geefGemPercVanBonnenInToekomst() {
		...
		return reductiebonLijst.stream().filter(x->x.getEinddatum().compareTo(LocalDate.now())>1).mapToInt(x->x.getPercentage()).average().getAsDouble();
	}

	// VRAAG4
	public List<LocalDate> geefUniekeEinddatums() {
		return reductiebonLijst.stream().map(x->x.getEinddatum()).distinct().collect(Collectors.toList());
