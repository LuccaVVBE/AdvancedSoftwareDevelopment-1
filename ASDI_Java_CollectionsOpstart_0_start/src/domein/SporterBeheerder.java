...
//	}
	...
	public List<Reductiebon> geefAlleReductiebonnenMetKortingsPercentageX(List<Integer> kortingspercentage) {
		...
		return sportersLijst.stream().map(x->x.getReductiebonLijst())//stream van collectie reductiebonnen
				.flatMap(Collection::stream)//stream van reductiebon
	...
	public void verwijderAlleSportersMetReductiebonMetPercX(int perc) {
		sportersLijst.removeIf(sporter->sporter.getReductiebonLijst().stream().anyMatch(x->x.getPercentage()==perc));
