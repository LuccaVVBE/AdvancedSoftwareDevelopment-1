...
public class DomeinController {
    ...
    public String opzettenAantalBierenPerSoort() {
    	//TODO
    	return bierWinkel.opzettenAantalBierenPerSoort().entrySet().stream().map(entry-> 
    	String.format("%s - %s%n", entry.getKey(), entry.getValue())).collect(Collectors.joining());
    }

    public String opzettenOverzichtBierenPerSoort() {
    	//TODO
    	return bierWinkel.opzettenOverzichtBierenPerSoort().entrySet().stream().map(entry-> 
    	String.format("%s - %s%n", entry.getKey(), entry.getValue())).collect(Collectors.joining());
    }
