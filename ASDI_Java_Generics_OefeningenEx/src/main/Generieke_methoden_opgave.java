...
//methode geefAlleElementenKleinerDan oproepen
    ...
    //------------------------------------------------------------------------
    private <E> E minimum(Collection<E> collectie) {
    	return collectie.stream().sorted().findFirst().orElse(null);
    }
    ...
    private <E extends Comparable<E>> List<E> geefAlleElementenKleinerDan(Collection<E> lijst, E grens) {
    	return lijst.stream().filter(elem->elem.compareTo(grens)<0).collect(Collectors.toList());
    }
}
